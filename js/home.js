...
$( '#directions_map' ).live( 'pagebeforeshow',function(event){
	...
	if (rdv != '')
	{
		$.post("https://ssl14.ovh.net/~taxibleu/client/in_app_calls.php", { map: 'true', cmd: cmd, rdv: rdv, com: com, idcourse: idcourse, cell: cell, pass: pass, dep: '13' }, function(data){
...
$( '#planning' ).live( 'pagebeforeshow',function(event){
	$.post("https://ssl14.ovh.net/~taxibleu/client/in_app_calls.php", { planning: 'true', tel: tel, pass: pass, dep: '13' }, function(data){
		$.mobile.loading( "show" );
...
$( '#history' ).live( 'pagebeforeshow',function(event){
	$.post("https://ssl14.ovh.net/~taxibleu/client/in_app_calls.php", { history: 'true', tel: tel, pass: pass, dep: '13' }, function(data){
		$.mobile.loading( "show" );
...
$( '#infos' ).live( 'pagebeforeshow',function(event){
	$.post("https://ssl14.ovh.net/~taxibleu/client/in_app_calls.php", { infos: 'true', pass: pass, dep: '13' }, function(data){
		$.mobile.loading( "show" );
...
$('#manage').live('pagecreate', function() {
	...
	$('#log').val(tel);
	$.post("https://ssl14.ovh.net/~taxibleu/client/billing.php", { taxi: taxi, pass: pass, dep: '13', mngid: mngid }, function(data){
...
function get_coords(position) 
{
	...
	$.post("https://ssl14.ovh.net/~taxibleu/client/insert_app_cab_geoloc.php?lat="+lat+"&lng="+lng, { taxi: taxi, tel: tel, email: email, pass: pass, dep: '13' }); 
}

function update()
{
	var dispo = $.sessionStorage.getItem('dispo');
	$.post("https://ssl14.ovh.net/~taxibleu/server/get_app_drive.php", { taxi: taxi, tel: tel, email: email, dispo: dispo, pass: pass, dep: '13', mngid: mngid, group: group }, function(data){ 
	...
	$.post("https://ssl14.ovh.net/~taxibleu/server/get_app_drive.php?cmd=1", { taxi: taxi, tel: tel, email: email, dispo: dispo, pass: pass, dep: '13', mngid: mngid, group: group }, function(data){ 
...
function dispo()
{
	$.post("https://ssl14.ovh.net/~taxibleu/client/dispo_app.php?check=1", { taxi: taxi, tel: tel, pass: pass, dep: '13' }, function(data){ 
		var display = '';
		if (data.dispo == 1)
		{
...
function Dispo_On()
{
	$.post("https://ssl14.ovh.net/~taxibleu/client/dispo_app.php?dispo=1", { taxi: taxi, tel: tel, pass: pass, dep: '13' });
...
function Dispo_Off()
{
	$.post("https://ssl14.ovh.net/~taxibleu/client/dispo_app.php?dispo=0", { taxi: taxi, tel: tel, pass: pass, dep: '13' }); 
...
function footer()
{
	$.post("https://ssl14.ovh.net/~taxibleu/client/footer_app.php", { dep: '13' }, function(data) {
		for (i=0; i<9; i++) {
...
{
	$.post("https://ssl14.ovh.net/~taxibleu/client/justify.php", { when: when, rdv: rdv, comments: comments, destadd: destadd, cell: cell, dep: '13', pass: pass, email: email }, function(data){
		$.mobile.loading( "show" );
		alert(data);
...
function directCall()
{
	...
	$.sessionStorage.setItem('query_string', query_string);
	$.post("https://ssl14.ovh.net/~taxibleu/server/diary_app_dcvp.php?dep=13", query_string, function(data){ 
...
{			
	...
	$.post("https://ssl14.ovh.net/~taxibleu/client/secure_xml.php", { lat: lat, lng: lng, dep: '13', pass: pass}, function(xml){
		...
		$(xml).find('marker').each(function(){
			...
			$.post("https://ssl14.ovh.net/~taxibleu/client/secure.php", { taxi: name, tel: address, rdvpoint: rdvpoint, helptaxi: taxi, helpname: helpname, helptel: tel, idcourse: idcourseUrg, num_req: num_reqUrg, dep: '13', pass: pass}, function(data){
...
{
	var idcourseUrg = $.sessionStorage.getItem('idcourseUrg');
	$.post("https://ssl14.ovh.net/~taxibleu/client/secure.php", { taxi: '', tel: '', rdvpoint: '', helptaxi: taxi, helpname: '', helptel: tel, idcourse: idcourseUrg, dep: '13', pass: pass, stopcall: 'true'}, function(data){
		$.mobile.pageContainer.pagecontainer("change", "#home", { transition: "slide"} );
	});
...
function Share()
{
	...
	var message = "Téléchargez l'app myTaxi 13 en suivant ce lien : http://www.taximedia.fr/stores.php?app=mytaxi&dep=13";
...
{
	var number = $('#telShare').val();
	var message = "Téléchargez l'app artisan taxi DCVP 13 en suivant ce lien : http://www.taximedia.fr/stores.php?app=dcvp&dep=13";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
		//alert('Message sent successfully');
		$('#smsReturn').empty().append('Message envoy&eacute; avec succ&egrave;s, Merci');
...
{
	var number = $('#telShare').val();
	var message = "Rendez-vous sur le WebService myTaxi 13 Hôtels & Restaurants en suivant ce lien : http://ecra.se/AA";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
		//alert('Message sent successfully');
		$('#smsReturn').empty().append('Message envoy&eacute; avec succ&egrave;s, Merci');
...
{
	var number = $('#telShare').val();
	var message = "Téléchargez l'app myTaxi 13 Pro sur les sores en suivant ce lien : http://www.taximedia.fr/stores.php?app=pro&dep=13  ou rendez-vous sur le WebService en suivant ce lien : http://ecra.se/3jt";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
		//alert('Message sent successfully');
		$('#smsReturn').empty().append('Message envoy&eacute; avec succ&egrave;s, Merci');
